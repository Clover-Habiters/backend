...
class SlackReportSenderTest {
	...
	private SlackReportSender slackReportSender;

	@BeforeEach
	void setUp() {
		slackReportSender = new SlackReportSender(TEST_WEBHOOK, threadPoolTaskExecutor);
	}
	...
	void slackNotificateTest() throws Throwable { // 비동기 실행되는 메서드(sendSlackMessage)는 어떻게 테스트해야하는가?
		...
		// When
		slackReportSender.sendReport(proceedingJoinPoint, request, exception);
